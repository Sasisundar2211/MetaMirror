...
# Initialize OpenRouter (which uses an OpenAI-compatible client)
...
if openrouter_api_key:
    try:
        ...
        logger.error("Failed to initialize Gemini model: %s", gemini_init_error)
        openrouter_init_error = str(e)
        logger.error("Failed to initialize OpenRouter client: %s", openrouter_init_error)
else:
    logger.warning("No GEMINI_API_KEY found in environment; Gemini disabled")
    logger.warning("No OPENROUTER_API_KEY found in environment; OpenRouter disabled")
...
class User(BaseModel):
    ...
    avatar_style: Optional[str] = "default" # Not used, but kept for schema compatibility
...
class UserCreate(BaseModel):
    ...
    avatar_style: Optional[str] = "default"
    preferred_provider: Optional[str] = "gemini"  # 'openai', 'gemini', or 'openrouter'
    preferred_provider: Optional[str] = "openrouter"
...
async def root():
    return {
        ...
        "providers": {
            ...
            "openrouter": {
                "key_present": bool(openrouter_api_key),
                "initialized": openrouter_client is not None,
                "init_error": openrouter_init_error
            }
        }
    }
...
async def chat_with_therapist(request: ChatRequest):
    ...
    provider = "openrouter"
    # Provide actionable errors depending on why OpenRouter isn't available
    if not openrouter_client:
        if not openrouter_api_key:
            raise HTTPException(status_code=503, detail="OpenRouter API key not set. Please set OPENROUTER_API_KEY in backend/.env and restart the server.")
        elif openrouter_init_error:
            raise HTTPException(status_code=503, detail=f"OpenRouter initialization failed: {openrouter_init_error}")
        else:
    ...
    try:
        ...
        if not openrouter_client:
            ...
            if not openrouter_api_key:
                ...
                raise HTTPException(status_code=503, detail="Gemini provider not initialized. Check logs for details.")
                raise HTTPException(status_code=503, detail="OpenRouter provider not initialized. Check logs for details.")
        ...
        try:
            ...
            # Surface the underlying message but avoid leaking sensitive data
            raise HTTPException(status_code=502, detail=f"Gemini service error: {str(e)}")
            raise HTTPException(status_code=502, detail=f"OpenRouter service error: {str(e)}")
