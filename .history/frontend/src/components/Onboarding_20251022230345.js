...
import { createUser, getProviderStatus } from "../utils/api";
...
const Onboarding = () => {
  ...
  const handleComplete = async () => {
    try {
      ...
      // Check backend/provider readiness first
      try {
        const status = await getProviderStatus();
        if (!status?.gemini?.initialized) {
          toast.error(`Backend Gemini not ready: ${status?.gemini?.init_error || 'initializing'}`);
          setIsSubmitting(false);
          return;
        }
      } catch (err) {
        toast.error(`Unable to reach backend at ${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}. Please ensure the backend is running.`);
        setIsSubmitting(false);
        return;
      }
